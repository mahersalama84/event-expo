...
import FollowingOptionsModal from "./FollowingOptionsModal";

const FollowingItem = (props: ProfileProps) => {
  const [active, setActive] = useState(false);

  const { getThemeColor } = useBaseTheme();
  const { openBaseBottomSheet, closeBaseBottomSheet } = useBaseBottomSheet();

  const selectFollowing = useFollowingsStore((state) => state.selectFollowing);
  ...
  const handlePress = () => {
    ...
    unselectFollower();
    unselectCustomer();
    selectFollowing(props.item);
    openBaseBottomSheet(<FollowingOptionsModal />);
  };
  return (
    <TouchableOpacity
      ...
      onPress={
        CustomersFunctions.isFollowing(props.item.id) ? handlePress : () => {}
      }
      style={[
    ...
    >
      ...
      >
        ...
        />
        <EvtView style={styles(getThemeColor).followingContainer}>
          <GetIcon
            icon={CustomersFunctions.AcceptedFollowingIconStatus(props.item.id)}
            variant="Bold"
...
const styles = (getThemeColor: getThemeColorType) =>
  StyleSheet.create({
    ...
    image: {
      flex: 1,
      width: "100%",
      height: AppConstants.FOLLOWING_ITEM_HEIGHT - AppConstants.SHADOW_WIDTH,
      borderRadius: Sizes.border.radius.md,
    },
    char: {
      flex: 1,
